...
slug: "/que-es-la-autenticacion-con-token"
date: 2015-02-19
tags: 
    - "💻 Desarrollo"
---
...
Por ello una de las nuevas tendencias en cuanto al desarrollo web moderno se refiere, es la autenticación por  medio de *Tokens* y que [nuestro backend sea un API RESTful sin información de estado](/como-implementar-una-api-rest-con-mongodb-node-js-usando-express-v4/), *stateless*.
...
![autenticación basada en Token](./autenticacion-basada-en-token.png)
...
- **Header**
La primera parte es la cabecera del token, que a su vez tiene otras dos partes, el tipo, en este caso un JWT y la codificación utilizada. Comunmente es el algoritmo *HMAC SHA256*, El contenido sin codificar es el siguiente:
```language-javascript
{
	"typ": "JWT",
    "alg": "HS256
...
- **Payload**
EL *Payload* está compuesto por los llamados [*JWT Claims*](http://self-issued.info/docs/draft-ietf-oauth-json-web-token.html#RegisteredClaimName) donde irán colocados la atributos que definen nuestro token. Exiten varios que puedes consultar aquí, los más comunes a utilizar son:
