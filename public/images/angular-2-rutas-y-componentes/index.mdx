...
slug: "/angular-2-rutas-y-componentes"
date: 2015-11-11
tags:
  - "💻 Desarrollo"
---
...
* **ROUTE_DIRECTIVES**, funciona de la misma manera, Contiene directivas como `RouterLink` y `RouterOutlet`. Las emplearemos en el `template` a la hora de *pintar* los links. 
...
Con este componente vamos a *jugar* un poco más y para ello vamos a utilizar parámetros de ruta con `RouteParams`, para ello tendremos que modificar un poco `app.js`, pero antes vayamos con este componente:
...
Ese parámetro lo llevará al mensaje que se *pinta* en el HTML con `routeParams.get('nombre')` siendo `nombre` el parámetro que tenemos que definir en `app.js`, donde hemos configurado las rutas.
