...
![Tipos de dispositivos y resoluciones](./resoluciones-y-tamanos-de-pantalla-responsive.png)
...
```javascript
<img src="img-mobile.jpg" 
     srcset="img-mobileHD.jpg 2x, img-tablet.jpg 768w" />
```
...
Vale, ya podemos utilizar varios tamaños y resoluciones de imágenes para hacer nuestra web más *Responsive*. 
...
Las **imágenes vectoriales** son las ideales para los logos, fuentes de iconos y sobre todo formas con colores planos. El formato más común para estos archivos es el `.SVG` de *Scalable Vector Graphics*. Al ser construidos vectorialmente en lugar de con píxeles, pueden escalar a múltiples formatos sin perder calidad.

Las **imágenes rasterizadas** son las ya conocidas por todos. Son las imágenes que podemos hacer con una cámara de fotos. Al contrario que las vectorizadas, si escalamos una imagen de un tamaño pequeño a otro más grande, perderemos calidad y la veremos *pixelada*.
...
Aunque usemos un sistema de compresión como JPG o PNG, siempre hay unos *kilobytes* de más que programas como Photoshop no terminan de comprimir eficientemente.
...
Esto significa que si queremos que en resolución *retina* se vea bien, siempre tendremos que tener una imagen de gran tamaño, que en un dispositivo sin esa resolución, no apreciaremos y por tanto estaremos desperdiciando recursos.
