...
![Chrome Flags](./chrome-flags-1.png)
...
Vamos a maquetar un sitio sencillo, compuesto por un *header*, un menú lateral, un contenido principal y un *footer*.
...
.header { color: white; background-color: #0D47A1; }
.footer { background-color: #2196F3; }
.main { background-color: #BBDEFB; }
.sidebar { background-color: #EEEEEE; }
```

Ahora pasamos a maquetar en formato de grilla o *Grid*.

Imaginemos un *Grid* compuesto por 3 columnas y 5 filas. En un *wireframe* sería algo así:

![Wireframe de un grid layout 3x5](./2015/10/wireframe-css-grid-layout.png)
...
El *header* y el *footer* ocuparán todo el ancho de la fila, es decir 3 columnas. Si la especificación estuviese completa podríamos decir lo siguiente:
...
```
Pero el selector `grid-column-span` aún no está implementado en Chrome, por lo que tenemos que hacer un pequeño *hack*. 
...
El *header* estará colocado en la fila 1, y el *footer* en la última fila, la 5. En CSS se traduce así:
...
Ahora queremos que el *sidebar* o menú esté en lo que sería la primera columna del *Grid* y la tercera fila:
...
Y por último, el contenido principal encerrado en el `<div>` con la clase `main` queremos situarlo en la tercera columna y tercera fila, al lado del *sidebar*:
...
![Página web con CSS Grid Layout](./2015/10/css-grid-layout.png)

## Y... ¿Esto es Responsive?

Por supuesto, y muy fácil. Sólo hay que pensar como sería el *Grid* en cada tamaño de pantalla y según la *media-query* cambiar el orden.

En este ejemplo no he aplicado el patrón *Mobile First*, pero si aplicamos *Graceful Degradation* podemos hacer algo tal que así:
...
![CSS Grid Layout Responsive](./responsive-grid-layout.png)

<YouTube videoId="CUzfyp2gMoU" />

## Siguientes pasos

Como he dicho, esto no es un estándar actualmente. Es un [borrador de propuesta](http://www.w3.org/TR/2015/WD-css-grid-1-20150917/), aún sin implementar en muchos navegadores, pero no está de más conocer que se está *cociendo* por ahí.
