...
Estos mensajes son muy útiles para entender qué cambios se realizaron en un proyecto, y por qué se realizaron.
...
para dar significado a los mensajes de los commits haciéndolos legibles tanto para las máquinas como
...
- **BREAKING CHANGE**: Un cambio que contenga esta palabra en el `footer` del mensajem o un signo `!` despues del tipo o scope,
  rompe la compatibilidad con versiones anteriores. Tendría correlación con una versión MAJOR siguiendo SemVer.
...
```shell
fix(ui): Corregir error en el componente Button
```
