...
En la [Firebase Summit 2022](https://firebase.google.com/summit/2022), Google anunció que el nuevo Firebase Hosting ya
estaba disponible en `preview`. La gran novedad es que permite desplegar aplicaciones
Server-Side Rendering (SSR) como podemos hacer con Next.js o Angular Universal.
...
utilizando Cloud Functions, pero era un poco engorrosa.

El equipo de Firebase ha trabajado para que ahora sea mucho más sencillo desplegar de esta
forma y con menos comandos.

En el momento de escribir este tutorial, el nuevo Firebase Hosting está en `preview` y es posible
...
El despliegue de este tipo de aplicaciones no era sencillo. Servicios como Vercel o Netlify lo
hacían muy fácil, pero Firebase Hosting no. Ahora, con el nuevo Firebase Hosting, es mucho más
sencillo.
...
¿Qué estamos haciendo aquí? Una página generada dinámicamente en el servidor, para ello
...
aunque es mejor que uses un servicio).
...
Firebase](https://console.firebase.google.com), haz click en el botón `Add project` y crea un nuevo proyecto. Llámalo como quieras.
...
Para ello, Firebase desde hace un tiempo nos da un set de emuladores de sus servicios (Firestore, Storage, Cloud Functions, etc...)
para probar el entorno en nuestro equipo local.
...
Si vas a `http://localhost:5002` verás la aplicación funcionando correctamente. La página
principal es estática asi que ahi no podemos probar el SSR, para ello dirígite a la URL por ejemplo
`http://localhost:5002/products/1` y verás que se renderiza correctamente.
...
La nueva funcionalidad de Firebase Hosting de servir contenido SSR, ejecuta por debajo Cloud Functions (tanto de Firebase como de Google Cloud)
y dentro de Google Cloud, crea un entorno en Cloud Run (Basicamente un Docker con Node.js y las funciones que ejecutan el SSR).
...
Ahi ir a la opción **Trigger** y en las opciones de **Authentication** seleccionar **Allow unauthenticated invocations**.
